...
public class MbpGenerator {
    ...
    private static SchemaEnum schemaEnum;
    private static String schemaPackage;

    @PostConstruct
    public void init() {
        schemaEnum = SchemaEnum.getByDbScheme(generatorConfig.getSchemaName());
        schemaPackage = "." + schemaEnum.getModulePackage() + ".";
    }
    ...
    public void genCodeBatch(GenSetting genSetting, List<String> tables) {
        ...
        // 调整生成文件配置(包含了包)
        userConfig.getOutputFiles().forEach(outputFileInfo -> {
            String fileType = outputFileInfo.getFileType();
            String outputLocation = outputFileInfo.getOutputLocation();
            if (CollUtil.newArrayList("Entity", "DTO", "VO").contains(fileType)) {
                outputFileInfo.setOutputLocation(outputLocation + "." + schemaEnum.getEntityPackage());
            } else if ("ServiceImpl".equals(fileType)) {
                outputFileInfo.setOutputLocation(StrUtil.replaceLast(outputLocation, ".service.impl", schemaPackage + "service.impl"));
            } else if (CollUtil.newArrayList("FeignClient", "FeignController").contains(fileType)) {
                outputFileInfo.setOutputLocation(outputLocation + "." + schemaPackage);
            } else {
                outputFileInfo.setOutputLocation(StrUtil.replaceLast(outputLocation, ".", schemaPackage));
            }
        });
        FastAutoGenerator
                ...
                .dataSourceConfig(
                    ...
                    builder.addInclude(String.join(",", tables))// 增加表匹配(内存过滤)
    ...
    private void configInjection(InjectionConfig.Builder builder, UserConfig userConfig, GenSetting genSetting) {
        ...
        builder.beforeOutputFile((tableInfo, objectMap) -> {
            ...
            List<String> superEntityColumns = entityStrategy.getSuperEntityColumns();
            // 判断是否包含父类字段
            vars.put("entityHasSuperEntityClass", tableInfo.getCommonFields().size() == superEntityColumns.size());
