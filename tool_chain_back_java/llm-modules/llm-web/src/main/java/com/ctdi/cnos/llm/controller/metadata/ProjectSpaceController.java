...
import com.ctdi.cnos.llm.feign.metadata.ProjectSpaceServiceClientFeign;
...
public class ProjectSpaceController {
	...
	private final ProjectSpaceServiceClientFeign serviceClient;
    ...
    public OperateResult<String> add(@Validated(Groups.ADD.class) @RequestBody ProjectSpaceDTO projectSpaceDTO) {
		return this.serviceClient.add(projectSpaceDTO);
    }
    ...
    public OperateResult<String> updateById(@Validated(Groups.UPDATE.class) @RequestBody ProjectSpaceDTO projectSpaceDTO) {
		return this.serviceClient.updateById(projectSpaceDTO);
    }
    ...
    public OperateResult<String> delete(@ApiParam(value = "项目空间信息表ID", required = true, example = "1")
                          @NotNull(message = "项目空间信息表ID不能为空") @RequestParam("id") Long id) {
