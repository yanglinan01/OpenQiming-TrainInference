...
function getHeaderName (currentPath, menuList) {
    console.log(1112,currentPath, menuList);
    ...
    let currentMenu = allMenus.find(item => item.path === currentPath);
    // console.log(22221,currentMenu);
    if(currentPath && currentMenu == undefined){
        if (currentPath == '/spaceManagementCenter/spaceDataset') {
            currentMenu = {
                path: currentPath,
                header: "projectSpace"
            };
        }else if (currentPath == '/spaceManagementCenter/datasetDetails') {
            currentMenu = {
                path: currentPath,
                header: "projectSpace"
            };
        }else if (currentPath == '/spaceModelTrain/parameters') {
            currentMenu = {
                path: currentPath,
                header: "projectSpace"
            };
    ...
    // console.log(2222,currentMenu);
    return  currentMenu.header;

    /* let urlMenu = ''
    if(currentPath && currentMenu == undefined){
        urlMenu = currentPath.split('/')[1].toString()
    }
    console.log(urlMenu);
    return currentMenu ? currentMenu.header : urlMenu; */
}
