    ...
    <div class="te-cont">
      ...
      <div class="template-card">
        <common-table
          ...
          :queryParam="villageconfig"
          @size-change="handleSizeChange"
          @current-change="handleCurrentChange"
        >
          <template #query>
              <el-form :model="villageconfig" ref="logform" label-width="120px" :inline="true">
          ...
          <template #setType="{ row }">
            <span :style="{ color:row.setType=='2' ? '#E8850B' : '#0AA64F'  }">
              {{ setTypeMap[row.setType] }}
              </span>
          </template>
          <template #operation="{ row }">
            <el-button type="primary" size="mini" @click="details(row)">查看</el-button>
            <el-button type="primary" size="mini" @click="cardDelete(row.id)" v-if="isshow == false">删除
            </el-button>
            <el-button type="primary" size="mini" @click="cardUpdate(row)" v-if="isshow == false">修改
            </el-button>
          </template>
        </common-table>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      tableColumns: [
        { prop: 'dataSetName', label: '数据集名称', slot: 'dataSetName', sortable: false, visible: true, search: true, searchType: 'input',align:'center' },
        { prop: 'dataType', label: '数据类型', slot: 'dataType', sortable: false, visible: true, search: false,align:'center' },
        { prop: 'createDate', label: '构建时间', slot: 'createDate', sortable: false, visible: true, search: false,align:'center'},
        { prop: 'setType', label: '类型', slot: 'setType', sortable: false, visible: true, search: false,align:'center' },
  ...
  methods: {
    queryList() {
      this.villageconfig.currentPage = 1;
      this.getList()
    },
    systemPrompt() {
      this.isshow = true,
    ...
    getList() {
      ...
      let params = {
        ...
        pageSize: that.villageconfig.pageSize,
        currentPage: that.villageconfig.currentPage,
      }
    ...
    handleCurrentChange(val) {
      this.villageconfig.currentPage = val;
      this.getList();
    },
    handleSizeChange(val) {
      this.villageconfig.pageSize = val;
      this.getList();
    },
    details(row) {
