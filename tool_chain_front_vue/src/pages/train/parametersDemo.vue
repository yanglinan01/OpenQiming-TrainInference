  ...
  <div>
    ...
    <div style="min-width: 1200px;">
      ...
      >
        ...
        <div class="blockBg">
          <p class="parameName">训练集群信息 <el-button type="primary"  @click="querySourceList()" :disabled="isButtonDisabled"  :class="{ 'disabled-button': isButtonDisabled }">策选集群</el-button></p>

          <div v-if="!!clusterLoading">
            <el-row class="trainTasks-dialog">
              <el-col :span="9" class="trainTasks-dialog-left"
              ><span>集群名称：{{ SourceSelectionoptions.clusterName }}</span></el-col
              >
              <el-col :span="9" class="trainTasks-dialog-left"
              ><span>集群省份：{{ SourceSelectionoptions.cloudProvince }}</span></el-col
              >
            </el-row>
            <el-row class="trainTasks-dialog">
              <el-col :span="9" class="trainTasks-dialog-left"
              ><span>CPU分配量：{{ SourceSelectionoptions.cpuCount }} <span v-if="SourceSelectionoptions.cpuCount">核</span></span></el-col
              >
              <el-col :span="9" class="trainTasks-dialog-left"
              ><span>内存分配量：{{ SourceSelectionoptions.memoryCount }} <span v-if="SourceSelectionoptions.memoryCount">MB</span></span></el-col
              >
            </el-row>
            <el-row class="trainTasks-dialog">
              <el-col :span="9" class="trainTasks-dialog-left"
              ><span>GPU分配量：{{ SourceSelectionoptions.gpuCount }} <span v-if="SourceSelectionoptions.gpuCount">卡</span></span></el-col
              >
              <el-col :span="9" class="trainTasks-dialog-left"
              ><span>GPU厂商：{{ SourceSelectionoptions.gpuVendor }}</span></el-col
              >
            </el-row>
            <el-row class="trainTasks-dialog">
              <el-col :span="9" class="trainTasks-dialog-left"
              ><span>GPU型号：{{ SourceSelectionoptions.gpuModel }}</span></el-col
              >
            </el-row>
          </div>
        </div>
...
<!--            <el-tab-pane label="训练集群信息" name="clusterInfo">-->
...
<!--              </div>-->
<!--            </el-tab-pane>-->
<!--            <el-tab-pane label="策选集群" name="selectCluster" >-->
<!--              <div v-loading="clusterDetailLoading">-->
<!--                <el-row class="trainTasks-dialog">-->
<!--                  <el-col :span="9" class="trainTasks-dialog-left"-->
<!--                  ><span>集群名称：{{ SourceSelectionoptions.clusterName }}</span></el-col-->
...
<!--                <el-row class="trainTasks-dialog">-->
<!--                  <el-col :span="9" class="trainTasks-dialog-left"-->
<!--                  ><span>CPU分配量：{{ SourceSelectionoptions.cpuCount }}</span></el-col-->
<!--                  >-->
<!--                  <el-col :span="9" class="trainTasks-dialog-left"-->
<!--                  ><span>内存分配量：{{ SourceSelectionoptions.memoryCount }}</span></el-col-->
<!--                  >-->
<!--                </el-row>-->
<!--                <el-row class="trainTasks-dialog">-->
<!--                  <el-col :span="9" class="trainTasks-dialog-left"-->
<!--                  ><span>GPU分配量：{{ SourceSelectionoptions.gpuCount }}</span></el-col-->
<!--                  >-->
<!--                  <el-col :span="9" class="trainTasks-dialog-left"-->
<!--                  ><span>GPU厂商：{{ SourceSelectionoptions.cloudName }}</span></el-col-->
<!--                  >-->
<!--                </el-row>-->
<!--                <el-row class="trainTasks-dialog">-->
<!--                  <el-col :span="9" class="trainTasks-dialog-left"-->
...
import {
  ...
  queryTrainModelList,
  queryTrainTaskList,
  trainTaskAdd,trainTaskAddDemo,
  clusterStrategyDetail
} from "@api/trainTask";
...
export default {
  ...
  data() {
    return {
      activeTab: 'clusterInfo', // 新增数据属性，用于控制当前激活的tab
      isKeyUp: false,
      isKeyUp1: false,
  ...
  methods: {
    ...
    querySourceList() {
      ...
      this.SourceSelectionoptions =
          {
            planId: "75520ca6-fb4d-4af0-a06c-6c6a27da265d",
            candidateId: "f26b821d-ea97-4461-b336-547a72e6a845",
            PUE: 1.5,
            CO2Data: 3,
            clusterId: "07fab3ec-8cb5-4f68-9462-555bcfb28228",
            clusterName: "贵州星海集群",
            costLevel: 1,
            cloudId: "dc3aceb44123134",
            gpuVendor: "HUAWEI",
            cloudProvince: "贵州",
            cpuCount: 1,
            gpuCount: 1,
      ...
      if (that.clusterLoading) {
        return;
      }
      that.clusterLoading = true;
      clusterStrategyDetail().then((res) => {
        if (res.success) {
          that.SourceSelectionoptions = res.data[0]
        }
      })
          .catch((err) => {
            console.log(err, "失败");
      ...
      that.$refs["donationForm"].validate(async valid => {
        if (valid) {
          ...
          let params = {
            ...
            type: that.trainType,
            planId :that.SourceSelectionoptions.planId,
            candidateId:that.SourceSelectionoptions.candidateId
            // classify: that.classify
          }
          // if(that.activeTab==='selectCluster'){
          //   params.planId = that.SourceSelectionoptions.resources[0].planId
          //   params.candidateId=that.SourceSelectionoptions.resources[0].candidateId
          //   // console.log(222222)
          // }
          // console.log(1111111111)
          trainTaskAddDemo(params).then((res) => {
            if (res.success) {
              that.submitLoading = false
              that.$message.success(res.message);
              this.$router.push({path: '/train/tasksDemo'})
            } else {
              that.submitLoading = false
...
//}
.disabled-button {
  background-color: #a0cfff; /* 灰色 */
  border-color: #a0cfff; /* 灰色 */
}
</style>
