...
import {
  ...
  deleteById, getTenants, deleteByIdDemo,
  getTrainTasksDetail,getTrainTasksDetailDemo,
  terminateById, trainTaskPublish,
  trainTasksQueryPage, trainTasksQueryPageDemo,
  trainTaskUpdateById, trainTaskUpdateByIdDemo
} from "@api/trainTask";
...
// console.log("x轴起始点：",this.echartPortINflowData.xAxis.startValue)
    ...
    getList() {
      ...
      trainTasksQueryPageDemo(params).then((res) => {
        if (res.success) {
    ...
    detail(id) {
      ...
      getTrainTasksDetailDemo(params)
          .then((res) => {
            if (res.success) {
    ...
    updateData() {
      ...
      trainTaskUpdateByIdDemo(params).then((res) => {
        if (res.success) {
          that.getList();
          that.$message.success(res.message);
        } else {
    ...
    deployStatusTextFilter(row, flag) {
      ...
      if (flag === 'color') {
        ...
        return !deployed ? "" : '(' + row.deployTaskList[0].statusName + ')'
      }
    },
