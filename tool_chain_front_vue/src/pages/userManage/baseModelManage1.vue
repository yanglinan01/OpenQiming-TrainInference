    ...
    <div class="trainTasksBlock">
      <div class="allTasksTableHead">
        <el-row :gutter="10" class="mb8">
          <el-form :model="queryParam" label-width="120px" :inline="true">
            ...
            <el-form-item>
              <div class="search-button">
                <el-button class="btn" type="primary" @click="queryModelList()" style="margin-left: 30px;height: 40px;">
                  查询
                </el-button>
              </div>
            </el-form-item>
      ...
      <el-table :data="allModelList" style="width: auto" v-loading="getListLoading">
        <el-table-column prop="name" label="模型名称" align="center">
        </el-table-column>
        <el-table-column prop="typeName" label="模型分类" align="center">
        </el-table-column>
        <el-table-column prop="belongName" label="模型归属" align="center">
        </el-table-column>
        <el-table-column prop="regionName" label="构建省份" align="center" width="300">
        </el-table-column>
        <el-table-column label="操作" width="300" align="center">
          <template slot-scope="scope">
            ...
            <span
                ...
                :class="{ 'disabled': scope.row.toDeleteDisabled }"
                @click="scope.row.toDeleteDisabled ? null : openAll(scope.row.id)">
              一键开放
            </span>
            <span
                :style="{ color: scope.row.status === 0 ? '#216EFF' : '#AFB4BA', fontSize: '100%', cursor: scope.row.status === 0 ? 'pointer' : 'not-allowed' }"
                :class="{ deployClickable: scope.row.status === 0 }"
                @click="scope.row.status === 0 && deployNow(scope.row.id)"
            >
              {{ scope.row.status === 0 ? '立即部署' : '已部署' }}
            </span>
          </template>
        </el-table-column>
      </el-table>
